<template>
  <div class="px-6 pb-6">
    ...
    <div class="relative group">
      ...
      <select
        v-model="selectedId"
        class="w-full bg-[#1e293b] border border-slate-800 text-slate-300 text-[11px] font-black uppercase tracking-widest rounded-none py-3.5 pl-11 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500 transition-all cursor-pointer hover:bg-slate-800 shadow-xl"
    ...
    >
      <div class="relative flex-1">
        ...
        <input
          ...
          class="w-full bg-[#1e293b] border border-slate-800 text-slate-300 text-[10px] font-mono rounded-none py-2 pl-9 pr-3 focus:outline-none focus:border-violet-500 transition-all"
          @keyup.enter="applyCustomId"
        >
      </div>
      <button 
        class="bg-violet-600 text-white p-2 rounded-none hover:bg-violet-700 transition-colors"
        @click="applyCustomId"
      >
        <Edit2 class="w-3 h-3" />
      </button>
...
const tenants = computed(() => tenantsList.value || [])

const selectedId = ref(tenantStore.selectedTenantId || '')

watch(() => tenants.value, (newList) => {
  if (newList.length > 0 && !selectedId.value) {
    selectedId.value = newList[0].id
    tenantStore.setTenant(newList[0].id, newList[0].name)
  }
}, { immediate: true })
