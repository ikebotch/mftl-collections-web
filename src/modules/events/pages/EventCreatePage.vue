      ...
      <div class="flex-1 p-10">
        ...
        <div v-if="currentStep === 2" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div class="flex items-center justify-between">
            <h3 class="text-xl font-bold text-slate-900">Recipient Funds</h3>
            <AppButton size="sm" variant="outline" @click="addFund">
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex-1 grid md:grid-cols-3 gap-4">
                <AppInput v-model="fund.name" label="Fund Name" placeholder="e.g. Medical Supplies" />
                <div class="md:col-span-2">
                   <AppInput v-model="fund.description" label="Description" placeholder="What will this specific fund be used for?" />
                </div>
                <AppInput v-model="fund.targetAmount" type="number" label="Target Amount" placeholder="0.00" />
              </div>
              <button 
            ...
            <div v-if="form.recipientFunds.length === 0" class="text-center py-12 border-2 border-dashed border-slate-100 rounded-2xl">
              <p class="text-slate-400 font-medium">No funds added yet. Every event needs at least one recipient fund.</p>
              <AppButton class="mt-4" size="sm" variant="secondary" @click="addFund">
                Add your first fund
              </AppButton>
            </div>
          </div>
        ...
        <div v-if="currentStep === 3" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div class="grid md:grid-cols-2 gap-10">
            <div class="space-y-6">
              <h3 class="text-lg font-bold text-slate-900">Currency Settings</h3>
              <AppSelect 
            ...
            <div class="space-y-6">
              ...
              <div class="space-y-3">
                <label v-for="method in paymentMethods" :key="method.id" class="flex items-center gap-3 p-4 rounded-xl border border-slate-100 bg-white hover:border-violet-200 cursor-pointer transition-all shadow-sm">
                  <input type="checkbox" v-model="form.acceptedMethods" :value="method.id" class="w-5 h-5 rounded-md border-slate-300 text-violet-600 focus:ring-violet-500">
                  <div class="flex-1">
        ...
        <!-- Step 4: Collectors -->
        <div v-if="currentStep === 4" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="w-16 h-16 rounded-3xl bg-violet-50 flex items-center justify-center text-violet-600 mb-6">
            <Users class="w-8 h-8" />
          </div>
          <h3 class="text-xl font-bold text-slate-900">Assign Collectors</h3>
        ...
        <div v-if="currentStep === 5" class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <section class="grid md:grid-cols-2 gap-8">
             <div>
               <h4 class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Event Details</h4>
               <p class="text-2xl font-bold text-slate-900">{{ form.title }}</p>
               <p class="text-slate-600 mt-2 font-medium">{{ form.description || 'No description provided.' }}</p>
               <p class="text-sm font-bold text-violet-600 mt-4">mftl.com/events/{{ form.slug }}</p>
             </div>
             <div>
               <h4 class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Schedule</h4>
               <p class="text-lg font-bold text-slate-900">{{ formatDate(form.startDate) }} — {{ form.endDate ? formatDate(form.endDate) : 'Indefinite' }}</p>
             </div>
          </section>

          <section>
            <h4 class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Target Funds</h4>
            <div class="grid md:grid-cols-2 gap-4">
              <div v-for="fund in form.recipientFunds" :key="fund.name" class="p-4 rounded-xl border border-slate-100 bg-slate-50">
                <p class="font-bold text-slate-900">{{ fund.name }}</p>
                <p class="text-sm text-emerald-600 font-bold mt-1">Target: {{ formatCurrency(fund.targetAmount, form.currency) }}</p>
              </div>
            </div>
          </section>
        </div>
...
async function submit() {
  ...
  try {
    ...
    const event = await createEventMutation.mutateAsync({
      ...
      recipientFunds: [] // We create them separately
    })
    
    // 2. Create recipient funds sequentially
    const fundPromises = form.recipientFunds
      .filter(f => f.name.trim())
      .map(f => recipientFundsService.create({
        eventId: event.id,
        name: f.name,
